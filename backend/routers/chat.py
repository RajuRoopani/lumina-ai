...
def chat_stream(
    ...
    try:
        ...
        # Save user message before streaming (session still open)
...
def compare_docs(
    ...
    def event_stream():
        done_payload = None
        try:
            yield f"data: {json.dumps({'event': 'progress', 'data': f'Comparing {len(docs)} documents...'})}\n\n"
            # Build instruction doc from user prompt so it guides report generation
            instruction_orm = DocumentORM()
            instruction_orm.id = "compare-instruction"
            instruction_orm.filename = "Comparison Instructions"
            instruction_orm.source_type = "text"
            instruction_orm.extracted_text = f"[COMPARISON INSTRUCTION FROM USER]\n{prompt}"
            comparison_docs = docs + [instruction_orm]
